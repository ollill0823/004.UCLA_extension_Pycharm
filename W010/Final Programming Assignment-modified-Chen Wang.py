...
Content:
    # 1e: Search for the word meaning from the dictionary website, and store the first definition.
            ...
            # 1e.2: Store all the information into the dictionary
...
while FileCheck :
    try:
        ...
        for i in list(dictionary.keys()):
            ...
            # get the first definition in the dictionary.com
            a_tags = soup_Dict.find('span', attrs={'class': 'one-click-content css-nnyc96 e1q3nk1v1'})
            Word_Temp = []
            # 1e.2: Store all the information into the dictionary
            for tag in a_tags:
                Word_Temp.append(tag.text)
                dictionary[i] = Word_Temp[0]
