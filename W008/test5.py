...
class MajorCharacter(Character):
    ...
    def recover(self, recoverhp):
        ...
        if self.hp + recoverhp >= self.maxhp:
            ...
            self.hp += recoverhp
        print('{var1}\'s Hp has been restored to {var3}/{var5}.'.format( var1 = self.getname(), var3 = self.gethp(), var5 = self.maxhp ))
...
# step 1 : input players information
...
# step 2 players's fight detail
## step 2a : input Boss and invader's  information
Putin= Boss()
...
while Putin.gethp() >0 :
    ...
    ## step 2c : fight detail: player's turn action
    for UKR in players:
        if Putin.gethp() == 0:
            break
        else:
            ...
            while YesorNoCheck:
                try:
                    ### step 2c.1 : give options (Attack/Charge/Shield) for players to choose an action
                    print('\033[94m\033[1m1. Attack\n2. Charge\n3. Shield\033[0m')
                    action = int(input('{}, what would you like to do? '.format(UKR.getname())))
                    ### step 2c.2 : based on the choose the players choose to develop the detail
    ...
    ## step 2d : fight detail: invader and Boss's turn action
